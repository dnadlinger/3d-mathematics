...
An dieser Stelle sei noch eine Eigenschaft von Transformationen in Bezug auf Normalvektoren erwähnt. Es ist in der 3D-Grafik oft erforderlich, bei der Transformation eines Objektes einen vorher berechneten Normalvektor ebenfalls zu transformieren. Es reicht nicht immer aus, einfach die gleiche Transformationsmatrix anzuwenden, beispielsweise bei der Scherung, die nicht winkeltreu ist.
...
Mithilfe dieser Formel transformieren wir nun die drei Basisvektoren $\vec{e_1} = $\textvec{1}{0}{0}, $\vec{e_2} = $\textvec{0}{1}{0} und $\vec{e_3} = $\textvec{0}{0}{1}. Für die Komponenten von $\vec{e_1'}$ erhalten wir die Gleichungen\footnote{$q$ ist eine Einheitsquaternion, also gilt $w^2 + x^2 + y^2 + z^2 = 1$!}:
\begin{equation}
\begin{split}
 {e_1'}_x
