...
% @Lz: bessere Variablennamen
\begin{align}
 \left| \vec{v_0} \right| = \left| \vec{v_1} \right| = \left| \vec{v_2} \right| = 0 \\
...
\begin{split}
 ...
 &= \left[ -\left| \vec{v_1} \right|^2; \vec 0 \right] \\
 &= -1
\end{split}
\end{equation}
...
Mithilfe dieser Formel transformieren wir nun die drei Basisvektoren $\vec x = $\textvec{1}{0}{0}, $\vec y = $\textvec{0}{1}{0} und $\vec z = $\textvec{0}{0}{1}. Für die Komponenten von $\vec x$ erhalten wir\footnote{$q$ ist eine Einheitsquaternion, also gilt $w^2 + x^2 + y^2 + z^2 = 1$!}:
\begin{equation}
\begin{split}
