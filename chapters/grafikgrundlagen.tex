...
Als nächstes folgt das \enquote{Herzstück} der 3D-Grafik, die \emph{Projektion} durch die \emph{Projection Matrix}. Diese Operation bildet den gesamten sichtbaren Raum in das \emph{kanonische Sichtvolumen} ab, in DirectX beispielsweise in einen Würfel zwischen den Punkten $\begin{pmatrix} -1 & -1 & 0 \end{pmatrix}^T$ und $\begin{pmatrix} 1 & 1 & 1 \end{pmatrix}^T$. Der Begriff Projektion mag etwas irreführend erscheinen, da das Ergebnis ja wieder ein Volumen ist, aber die $z$-Koordinate wird nur noch zur Bestimmung der Sichtbarkeit gebraucht, die Position der Vertices am Bildschirm wird durch die $x$- und $y$-Koordinate ausgedrückt. Die Verwendung eines kanonischen Sichtvolumens hat den Vorteil, dass alle nachfolgenden Verarbeitungsschritte komplett unabhängig von der Art der Projektion durchgeführt werden können. Dadurch lässt sich die Umsetzung in Hardware viel effizienter gestalten. 

Nach diesem Schritt werden alle Bereiche verworfen, die sich außerhalb des Sichtvolumens befinden. Dieser Vorgang wird \emph{Clipping} genannt, die Koordinaten nach der Multiplikation mit der Projection Matrix werden daher auch als \emph{Clipping-Koordinaten} bezeichnet.
...
World, View und Projection Matrix können in eine Matrix kombiniert werden. 
